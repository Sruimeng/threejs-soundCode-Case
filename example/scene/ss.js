...
vec4 envMapTexelToLinear( vec4 value ) { return RGBMToLinear( value, 7.0 ); }
	...
	uniform int maxMipLevel;
        ...
        vec3 getLightProbeIndirectIrradiance( /*const in SpecularLightProbe specularLightProbe,*/ const in GeometricContext geometry, const in int maxMIPLevel ) {
            ...
            vec3 worldNormal = inverseTransformDirection( geometry.normal, viewMatrix );
                ...
                vec3 queryVec = vec3( flipEnvMap * worldNormal.x, worldNormal.yz );
                vec3 rotated = rotateAroundYInDegrees(vec3( queryVec ),-uYRotation);
                vec4 envMapColor = textureCubeUV( envMap, rotated, 1.0 );
...
float computeSpecularOcclusion( const in float dotNV, const in float ambientOcclusion, const in float roughness ) {
	...
	vec2 cubeToUV( vec3 v, float texelSizeY ) {
        vec3 yRotation=transformDirection(v,-uYRotation);
		vec3 absV = abs( yRotation );
